...
export class JwtClient {
  private key =
    inject(Configurator).entries?.jwt?.key ?? env<string>('JWT_KEY')!;

  constructor(private encrypter: Encrypter) {}

  private createHeader(): string {
    return this.encrypter.base64Encode(JSON.stringify({
      alg: 'HS256',
      typ: 'JWT',
    }));
  }
  ...
  public generateToken(payload: unknown): string {
    ...
    return `${header}.${payloadData}.${signature.digest('base64').replaceAll(/\+\//g, '-').replaceAll('=', '')}`;
  }
}
